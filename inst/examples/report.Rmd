...
p <- ggplot(dfs, aes(x = names, y = document_length))
  p <- p + geom_bar(stat = "identity")
  p <- p + coord_flip()
  v <- 10^(1:max(na.omit(round(log10(max(dfs$document_length))))))
  p <- p + scale_y_log10(breaks = v, labels = v)
  p <- p + ylab("Document length (n)") + xlab("")
  p <- p + ggtitle("Document length")
  p2 <- p
