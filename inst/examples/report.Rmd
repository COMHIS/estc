...
The top authors can be defined by title count (regardless of number of hits in each title; left) or by the total sum of query hits across all documents (right).
...
```{r topauthors2a, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
...
```{r topauthors2b, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
dfs <- dfs %>% dplyr::filter(!is.na(names))
save(dfs, df, file = "tmp.RData")
```

```{r topauthors2c, echo=FALSE, message=FALSE, warning=FALSE}
dfs <- dfs %>% dplyr::summarise(count = sum(hits, na.rm = T))
```

```{r topauthors2d, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
dfs <- dfs %>% arrange(desc(count))
```

```{r topauthors2e, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
dfs <- dfs %>% head(ntop) 
```

```{r topauthors3, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
if (nrow(dfs) > 0) {
  ...
  p2 <- ggplot() + ggtitle("No data for this selection")
}
```


```{r topauthors4, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
grid.arrange(p1, p2, nrow = 1)
```
...
```{r toptitles2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=5}
df <- df0
v <- 10^(1:max(na.omit(round(log10(max(table(df$title)))))))
...
dfs <- dfs %>% mutate(hits.per.edition = count/n)
...
The top publishers can be defined by title count (regardless of number of hits in each title; left) or by the total sum of query hits across all documents (right).
