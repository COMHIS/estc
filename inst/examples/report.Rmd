...
idfile   <- params$id.file
ids <- as.character(unlist(read.csv(idfile), use.names = FALSE))
#ids <- as.character(sample(df0[, idfield], 1e4)) # Random IDs
...
Entry IDs (n=`r length(ids)`) are read from the file:  `r params$id.file`

Preprocessed data is from the file: `r params$data`
...
### Histogrammi jossa kaikki teokset

Histogrammi minkä muuttujan suhteen ? Vai tarkoitatko barplottia joka
näyttää aikajanalla dokumenttien määrän kunakin vuonna /
vuosikymmenenä?
