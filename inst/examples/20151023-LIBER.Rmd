...
library(ggplot2)
library(dplyr)
dfs <- filter(df, !is.na(author_birth) & !is.na(author_death))
...
p <- p + ggtitle("Top history authors in ESTC")
print(p)
```
...
p <- p + ggtitle("Top author life spans")
print(p)
```
...
p2 <- p2 + ggtitle("Title counts over time for selected authors")
p2
```
...
p <- p + ggtitle("Title count versus paper consumption (authors)")
print(p)	 
```
...
dfs <- dfs[-c(grep("jean", dfs$author_unique), grep("pierre", dfs$author_unique), grep("capriata", dfs$author_unique), grep("mesurier", dfs$author_unique)),]
...
p <- p + scale_color_manual(values = c("darkgray", "black"))
p <- p + guides(color = "none")
p <- p + ggtitle("Title count versus paper consumption (places)")
print(p)	 
```
...
```{r 20151023LIBER-10b, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=4, fig.width=8}
...
p <- p + ggtitle("Octavo vs Folio paper consumption among top authors")
print(p)
```
