...
theme_set(theme_bw(20))
dfs <- df %>% group_by(author.unique) %>% summarize(titles = n(), paper = sum(paper.consumption.km2, na.rm = T))
...
#dfs <- df %>% filter(publication.place %in% top) %>% group_by(publication.place) %>% summarize(titles = n(), paper = sum(paper.consumption.km2, na.rm = T))
#dfs <- dfs %>% filter(as.character(author.unique) %in% top)
