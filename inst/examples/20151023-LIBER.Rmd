...
top <- na.omit(names(rev(sort(table(dfs$author_unique)))))[1:20]
...
rm.inds <- c(grep("Jean", dfs$author_unique), grep("Pierre", dfs$author_unique), grep("Capriata", dfs$author_unique), grep("Mesurier", dfs$author_unique), grep("Bouhours", dfs$author_unique))
dfs <- dfs[-rm.inds,]
...
```{r 20151023LIBER-topplace, echo=FALSE, message=FALSE, cache=FALSE, fig.height=6, fig.width=6}
...
```{r 20151023LIBER-docsizecomp1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
dfs <- df %>% select(gatherings, publication_country) %>% group_by(gatherings, publication_country) %>% tally() %>% filter(publication_country %in% c("Scotland", "Ireland", "USA"))
p <- ggplot(dfs, aes(x = gatherings, y = publication_country, size = n)) +
     geom_point()
print(p)
```
...
```{r 20151023LIBER-docsizecomp2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
dfs <- df %>% select(pagecount, publication_country) %>% filter(publication_country %in% c("Scotland", "Ireland", "USA"))
p <- ggplot(dfs, aes(x = pagecount)) +
     geom_histogram() + facet_grid(publication_country ~ .) + scale_x_log10()
print(p)
```

Gatherings vs. pagecounts per country

```{r 20151023LIBER-docsizecomp3, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=10}
dfs <- df %>% select(pagecount, gatherings, publication_country) %>% filter(publication_country %in% c("Scotland", "Ireland", "USA")) %>% group_by(pagecount, gatherings, publication_country) %>% tally()
p <- ggplot(dfs, aes(x = pagecount, y = gatherings, size = n)) +
     geom_point() + facet_grid(publication_country ~ .) + scale_x_log10()
print(p)
```
...
```{r 20151023LIBER-11, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=5, fig.width=9}
...
```{r 20151023LIBER-12, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, fig.height=6, fig.width=10}
df2 <- df
...
# Ireland or Scotland
#dfs <- df.preprocessed %>% filter(publication_year > 1470) %>% group_by(publication_country) %>% filter(is.na(gatherings), publication_country %in% c("USA")) %>% arrange(publication_year) %>% select(publication_year, author_name, publication_place, pagecount, title, gatherings.original); write.table(dfs, file = "output.tables/USA-missing-gatherings.tab", quote = F, row.names = F)

#dfs <- df.preprocessed %>% filter(publication_year > 1470) %>% group_by(publication_country) %>% filter(publication_country %in% c("USA")) 
# Proportion of docs without gatherings
#> mean(is.na(dfs$gatherings))
#[1] 0.2446465
# Estimated effect on page count (assuming missing docs have same size
# distribution than other docs)
#> 1/mean(!is.na(dfs$gatherings))
#[1] 1.323883

#> df.preprocessed %>% filter(publication_year > 1470) %>% group_by(publication_#country) %>% filter(publication_country %in% c("Scotland", "Ireland", "USA")) %>% summarize(missing_gatherings = mean(is.na(gatherings)))
#Source: local data frame [3 x 2]
#  publication_country missing_gatherings
#               (fctr)              (dbl)
#1             Ireland         0.03134240
#2            Scotland         0.01995012
#3                 USA         0.24464652
```
