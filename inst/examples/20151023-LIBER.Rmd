...
dfs <- df %>% filter(author_unique %in% selected.authors) %>%
	 group_by(author_unique, publication_year) %>%
	 tally() 
theme_set(theme_bw(11))
p <- ggplot(dfs, aes(x = publication_year, y = n, color = author_unique))
p <- p + geom_line()
print(p)	 
```
...
theme_set(theme_bw(20))
dfs <- df %>% group_by(author_unique) %>% summarize(titles = n(), paper = sum(paper.consumption.km2, na.rm = T))
dfs$highlight <- rep("darkgray", nrow(dfs))
dfs$highlight[dfs$author_unique %in% selected.authors] <- "red"
dfs <- dfs %>% filter(as.character(author_unique) %in% top)
p <- ggplot(dfs, aes(x = titles, y = paper, color = highlight)) +
       geom_text(aes(label = author_unique)) +
       scale_color_manual(values = c("darkgray", "black")) +
...
```{r 20151023LIBER-7, echo=FALSE, message=FALSE, cache=FALSE, fig.height=5, fig.width=8}
dfs <- df %>% filter(country == "Scotland")
p <- top_plot(dfs, "author_unique", ntop = 20, highlight = selected.authors)
p <- p + scale_fill_manual(values = c("darkgray", "black"))
p <- p + ylab("Title count")
p <- p + guides(fill = "none")
print(p)
...
```{r 20151023LIBER-9, echo=FALSE, message=FALSE, cache=FALSE, fig.height=4, fig.width=8}
dfs <- df %>% filter(topic == "Broadsides" & !is.na(publication_year) & publication_year > 1470) %>%
