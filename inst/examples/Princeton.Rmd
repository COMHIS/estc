...
dfs <- dfs %>% group_by(topic, publication_decade) %>% summarize(paper = sum(paper.consumption.km2, na.rm = TRUE), documents = n())
...
dfs <- dfs %>% group_by(country, publication_decade) %>% summarize(paper = sum(paper.consumption.km2, na.rm = TRUE), documents = n())
theme_set(theme_bw(20))
