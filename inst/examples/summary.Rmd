...
tab <- cbind(available = colSums(!is.na(df)),
      unique = apply(df, 2, function (x) {length(unique(x))}))
tab <- tab[order(tab[, "available"]), ]
kable(tab)
```
...
dfs <- df %>% filter(is.na(author_unique) & !is.na(author_name))

# Authors with many birth years
births <- lapply(split(dfs$author_birth, dfs$author_name), unique)
many_births <- lapply(births[names(which(sapply(births, function (x) {length(unique(na.omit(x)))}) > 1))], function (x) {sort(unique(na.omit(x)))})

# Authors with many death years
deaths <- lapply(split(dfs$author_death, dfs$author_name), unique)
many_deaths <- lapply(deaths[names(which(sapply(deaths, function (x) {length(unique(na.omit(x)))}) > 1))], function (x) {sort(unique(na.omit(x)))})

dfs <- subset(dfs, author_name %in% union(names(many_births), names(many_deaths)))
dfs <- dfs[, c("author_name", "author_birth", "author_death")]
dfs <- dfs[!duplicated(dfs),]
dfs <- dfs[order(dfs$author_name), ]
...
#p <- ggplot(dfs)
#p <- p + geom_segment(aes(y = author_name, yend = author_name, x = author_birth, xend = author_death), #size = 2) 
...
print(p)
#publications.annual <- tapply(df$unity, list(df$publication_year, df$place), sum)
...
top10 <- names(sort(table(df$publisher), decreasing = TRUE))[1:10]
...
Top-`r ntop` publication places are shown together with the number of documents. This info is available for `r sum(!is.na(df$place))` documents (`r round(100*mean(!is.na(df$place)))`%). There are `r length(unique(str_trim(unlist(strsplit(as.character(df$place), ";")))))` unique publication places. Overall `r round(100*mean(!is.na(df$latitude) & !is.na(df$longitude)), 1)`% of the places could be matched to geographic coordinates (from the [Geonames](http://download.geonames.org/export/dump/) database).
...
kable(data.frame(n = rev(sort(table(df$country)))), caption = "Publication country counts")
```

```{r summaryplacemissinggeo, echo=FALSE, results='asis'}
inds <- is.na(df$latitude) & is.na(df$longitude)
p <- top_plot(df[inds, ], "place")
...
dfs <- select(df, starts_with("document.dimension")) %>% filter(!is.na(cm2) & !is.na(gatherings))
dfs <- dfs[, c("gatherings", "cm2")]
dfm <- melt(table(dfs))
...
```{r summary-surfacearea, fig.height=30, fig.width=10, echo=FALSE}
p <- ggplot(df, aes(x = area)) 
p <- p + geom_histogram() 
p <- p + xlab("Document surface area (cm2; log10)")
...
p <- ggplot(df4, aes(x = gatherings, y = height))
p <- p + geom_point(aes(size = n))
p <- p + geom_point(data = unique(df3), aes(x = gatherings, y = gathering.height.estimate), color = "red")
...
```{r summary-histograms, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=12}
...
```{r summary-bars, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=12}
...
for (field in factor.fields) {
  df$fieldtoplot <- df[[field]]
  p <- top_plot(df, "fieldtoplot", ntop)
