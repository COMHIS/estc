...
dfs <- filter(df, author.unique %in% top10)
dfs <- group_by(dfs, author.unique, publication.year) %>%
    summarize(paper.consumption.km2 = sum(paper.consumption.km2, na.rm = TRUE), n = n()) 
p <- ggplot(dfs, aes(x = publication.year, y = paper.consumption.km2)) 
p <- p + geom_bar(stat = "identity") 
p <- p + facet_grid(author.unique ~ .)
...
dfs <- group_by(dfs, publication.publisher, publication.year) %>% summarize(ndoc = n())
p <- ggplot(dfs, aes(x = publication.year, y = ndoc)) 
p <- p + geom_bar(stat = "identity") 
...
dfs <- filter(df, publication.publisher %in% top10)
dfs <- group_by(dfs, publication.publisher, publication.year) %>%
    summarize(paper.consumption.km2 = sum(paper.consumption.km2, na.rm = TRUE), n = n()) 
p <- ggplot(dfs, aes(x = publication.year, y = paper.consumption.km2)) 
p <- p + geom_bar(stat = "identity") 
p <- p + facet_grid(publication.publisher ~ .)
...
Publication year is available for `r sum(!is.na(df$publication.year))` documents (`r round(100*mean(!is.na(df$publication.year)))`%). The publication years span `r paste(range(na.omit(df$publication.year)), collapse = "-")`
...
Document size (area) info in cm2 is available for `r sum(!is.na(df$document.dimension.cm2))` documents (`r round(100*mean(!is.na(df$document.dimension.cm2)))`%). Estimates of document size (area) info in gatherings system are available for `r sum(!is.na(df$document.dimension.gatherings.estimated))` documents (`r round(100*mean(!is.na(df$document.dimension.gatherings.estimated)))`%). 

```{r summarysize, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
df <- df.preprocessed
p <- ggplot(df, aes(x = document.dimension.gatherings.estimated)) 
p <- p + geom_bar()
n <- nchar(max(na.omit(table(df$document.dimension.gatherings.estimated))))
...
Compare gatherings and page counts. Page count information is estimated for `r sum(!is.na(df$document.pages.total.orig)) - sum(!is.na(df$document.pages.total))` documents and updated (changed) for `r sum(!df$document.pages.total.orig == df$document.pages.total, na.rm = T)` documents. 

```{r summarypagecomp, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=7}
dfs <- select(df, document.pages.total, document.dimension.gatherings.estimated) 
dfs$document.pages.total <- as.numeric(gsub(" pages", "", dfs$document.pages.total))
dfs <- dfs %>% filter(!is.na(document.pages.total) & !is.na(document.dimension.gatherings.estimated))
dfg <- group_by(dfs, document.pages.total, document.dimension.gatherings.estimated) %>% tally()
names(dfg) <- c("pages", "gatherings", "documents")
dfg$gatherings <- factor(dfg$gatherings, levels = levels(df$document.dimension.gatherings.original))
...
dfs <- df
dfs$document.pages.total.orig <- as.numeric(as.character(gsub(" pages", "", dfs$document.pages.total.orig)))
p <- ggplot(dfs, aes(x = document.pages.total.orig, y = document.pages.total)) 
p <- p + geom_point()
p <- p + scale_x_log10()
p <- p + scale_y_log10()
...
df2 <- filter(df, !is.na(document.dimension.height.original) | !is.na(document.dimension.width.original))
df2 <- df2[!is.na(as.character(df2$document.dimension.gatherings.original)),]
df3 <- filter(df2, !is.na(document.dimension.height.original))
ss <- sheet_sizes()
df3$gathering.height.estimate <- ss[match(df3$document.dimension.gatherings.original, ss$gatherings),"height"]
df4 <- df3 %>% group_by(document.dimension.gatherings.original, document.dimension.height.original) %>% tally()
p <- ggplot(df4, aes(x = document.dimension.gatherings.original, y = document.dimension.height.original))
p <- p + geom_point(aes(size = n))
p <- p + geom_point(data = unique(df3), aes(x = document.dimension.gatherings.original, y = gathering.height.estimate), color = "red")
...
df2 <- filter(df, !is.na(document.dimension.gatherings.original) & (!is.na(document.dimension.height.original) | !is.na(document.dimension.width.original))) %>% group_by(document.dimension.gatherings.original) %>% 
  summarize(mean.height = mean(document.dimension.height.original, na.rm = T),
	    median.height = mean(document.dimension.height.original, na.rm = T),
    	    mean.width = mean(document.dimension.width.original, na.rm = T), 
	    median.width = mean(document.dimension.width.original, na.rm = T), 
	    n = n()) %>%
  filter(n > 10 & !is.na(as.character(document.dimension.gatherings.original)))
