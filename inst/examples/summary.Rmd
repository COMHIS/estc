...
p <- top_plot(df[inds, ], "publication_place")
...
dfs <- select(df, starts_with("document.dimension")) %>% filter(!is.na(area) & !is.na(gatherings))
...
```{r summary-surfacearea, fig.height=10, fig.width=10, echo=FALSE}
...
```{r summary-histograms, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=8}
