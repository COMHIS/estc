...
year <- 500
dfs <- filter(df.preprocessed, author_birth < year)
...
dfss <- dfs %>% filter(author %in% top) %>%
	...
	summarize(paper = sum(paper.consumption.km2))
...
df1700 <- filter(df.preprocessed, publication_year >= 1700 & publication_year <= 1800) 
...
dfa <- filter(df.preprocessed, author %in% authors) 
...
for (ctr in c("Scotland", "USA")) {
  dfs <- df.preprocessed %>% filter(country == ctr) %>% 
...
df <- df.preprocessed %>% filter(publication_year >= 1750 & publication_year <= 1799)
