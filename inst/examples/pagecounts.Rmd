...
for (i in 1:nrow(comptab)) {
  ...
  if (is.na(dfs$volcount) & !is.na(dfs$volnumber)) {
    ...
    comptab[i, "ecco"] <- sum(subset(ecco, id == myid)$totalPages[dfs$volnumber], na.rm = TRUE)
  } else {
...
for (i in 1:nrow(comptab)) {
  ...
  if (is.na(dfs$volcount) & !is.na(dfs$volnumber)) {
    ...
    #if (nrow(ecco.set) == 1) {
    #  # If ECCO only lists a single volume then use that pagecount
    #  comptab[i, "ecco"] <- ecco.set$totalPages
    #} else {
