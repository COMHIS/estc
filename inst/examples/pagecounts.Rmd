...
#write.csv(coms, file = "ecco_estc_common_ids.csv", quote = FALSE, row.names = FALSE)
```

There are `r sum(as.character(ecco$ESTCID) %in% df$id)` ECCO documents (`r round(100*mean(as.character(ecco$ESTCID) %in% df$id), 1)`%) with a match in ESTC in our data based on the system control number.
...
```{r ecco2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
library(ggplot2)
theme_set(theme_bw(20))
dfs <- filter(df, !is.na(pagecount.ecco) & !is.na(pagecount))
