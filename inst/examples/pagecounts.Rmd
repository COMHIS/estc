...
df <- df.preprocessed
  ...
  p <- ggplot(dff2, aes(y = gatherings, x = pages.total)) 
...
library(dplyr)
df2 <- df %>% group_by(publication_decade, gatherings) %>% summarize(na = sum(is.na(pages.total.orig) | (!pages.total.orig == pages.total)))
df2$na[df2$na == 0] <- NA
library(ggplot2)
theme_set(theme_bw(15))
...
library(dplyr)
df2b <- df %>% group_by(publication_decade, gatherings) %>% summarize(na = sum(is.na(pages.total)))
