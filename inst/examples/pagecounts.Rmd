...
o <- subset(df, pagecount.from.ecco)$original_row
comptab <- cbind(
...
df2 <- df
df2 <- subset(df, !pagecount.from.ecco)
o <- df2$original_row[order(abs(df2$pagecount.ecco - df2$pagecount) / (df2$pagecount.ecco + df2$pagecount), decreasing = TRUE)];
comptab <- cbind(
	estc.id = as.character(df2[match(o, df2$original_row),"system_control_number"]),
	ecco.id = as.character(df2[match(o, df2$original_row), "ecco.id"]),	
	gatherings = df2[match(o, df2$original_row),"gatherings"],
	physical_extent = df.orig[match(o, df.orig$original_row), "physical_extent"],	
	estc = df2[match(o, df2$original_row),"pagecount"],
	ecco = df2[match(o, df2$original_row),"pagecount.ecco"],	
	year = df2[match(o, df2$original_row),"publication_year"],	
	author = df2[match(o, df2$original_row),"author"],	
	title = df2[match(o, df2$original_row),"title"]	
	)
# Sum up all ECCO hits
for (i in 1:nrow(comptab)) {
  comptab[i, "ecco"] <- sum(subset(ecco, id == comptab[i,"ecco.id"])$totalPages, na.rm = TRUE)
}
comptab <- comptab[order(abs(comptab[, "estc"] - comptab[, "ecco"]), decreasing = TRUE),]
write.table(comptab, file = "~/tmp/estc_ecco_pagecount_mismatches.csv", quote = F, row.names = F, sep = "\t")
...
There are `r sum(as.character(ecco$id) %in% df$id)` ECCO documents (`r round(100*mean(as.character(ecco$id) %in% df$id), 1)`%) with a direct match in ESTC in our data based on the system control number. The total page count for the matched ESTC documents is `r round(100 * total.match, 1)`% of the total page count over the same ECCO documents.
