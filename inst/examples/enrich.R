...
geoc <- get_geocoordinates(df.preprocessed$publication_place, geonames, places.geonames)
# Remove earlier versions of these fields
...
#df.preprocessed.preprocessed$publication_year <- approximate_pubyear(df.preprocessed.preprocessed)

# print("Estimate the number of physical items for each document")
# df.preprocessed.preprocessed$document.items <- estimate_document_items(df.preprocessed.preprocessed) # "Physical items per document"
