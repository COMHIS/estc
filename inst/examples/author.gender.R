...
first <- sapply(as.character(df$author.name), function (x) {y <- unlist(strsplit(x, ", ")); if (length(y)>1) y[[2]] else NA})
g <- get_gender(first)
df$author.gender <- g$gender
...
tab <- data.frame(list(name = first, gender = df[, "author.gender"]))
