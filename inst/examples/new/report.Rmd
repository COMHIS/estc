...
df.orig <- readRDS(datafile.preprocessed) # Keep the complete data
...
```{r toppublishersA, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
df <- df0
field <- "publisher"
p <- NULL
...
```{r toppublishersA2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
# Use unnormalized frequency for now
df$hits <-  unlist(custom.ids[match(df$id, custom.ids$id), "freq"], use.names = F)
df$names <- df[[field]]
```

```{r toppublishersA3, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
dfs <- df %>% group_by(names) %>%
              filter(!is.na(names)) %>% 
...
```{r toppublishersB, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
# Limit title length
dfs$names <- substr(as.character(dfs$names), 1, nchar)
```

```{r toppublishersC, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
